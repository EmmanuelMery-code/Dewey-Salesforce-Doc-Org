...
| 3 | **Détection d'auto-récursion Apex** (APEX-REL-002) | `apex_analyzer._detect_self_recursive_methods` | Extrait chaque méthode par comptage d'accolades (`_extract_method_bodies` → `ApexMethod`: nom, types des paramètres, corps), puis recherche un auto-appel dans son propre corps. Trois filtres successifs : (a) **récepteur** — seuls les appels non qualifiés, `this.methode(...)` et `MaClasse.methode(...)` comptent, un appel qualifié vers une homonyme d'une autre classe ou instance (`new AccountService().completeSirenisation(...)`) est ignoré ; (b) **signature** (`_call_targets_same_signature`) — l'arité des arguments doit correspondre, et si plusieurs surcharges partagent cette arité, les types inférés des arguments (`_infer_argument_type` : littéraux, `new X()`, casts, variables locales et paramètres) doivent égaler ceux de la méthode appelante ; un type indécidable fait **s'abstenir** la règle. La délégation `sync(id)` → `sync(id, false)` n'est donc plus signalée. Le stripping est appelé avec `keep_string_markers=True` pour que `f('a')` ne se confonde pas avec `f()` lors du comptage d'arité ; (c) **garde** — ignoré si la méthode **consulte** une garde de réentrance (`_find_recursion_guards` + `_body_is_guarded`) : champ `static` non `final` de type `Boolean`/`Integer`/`Set<>`/`Map<>`, ou identifiant au nom parlant (`isRunning`, `bypass`, `TriggerBypass.skipAll`, …), effectivement lu dans un `if`/`while` ou via `contains(`. La garde est évaluée **méthode par méthode** : une garde posée dans une autre méthode n'arrête pas une récursion interne. Chaque finding porte la **signature** incriminée (`walk(Id, Integer)`) et non le seul nom. |
| 4 | **Détection de récursion de trigger after-save** (TRIG-REL-001) | `apex_analyzer._detect_trigger_after_save_recursion` | Analyse la déclaration du trigger (`trigger X on Y (after insert, ...)`) puis recherche par regex si `Trigger.new`/`Trigger.newMap` (ou un alias assigné via `Object x = Trigger.new`) fait l'objet d'un DML direct — signature classique de boucle infinie. Ignoré si le corps du trigger consulte une garde de réentrance (même logique `_body_is_guarded` que la règle n°3). |
...
| 3 | **Apex self-recursion detection** (APEX-REL-002) | `apex_analyzer._detect_self_recursive_methods` | Extracts each method via brace counting (`_extract_method_bodies` → `ApexMethod`: name, parameter types, body), then looks for a self-call inside its own body. Three successive filters: (a) **receiver** — only unqualified calls, `this.method(...)` and `MyClass.method(...)` count; a qualified call to a same-named method on another class or instance (`new AccountService().completeSirenisation(...)`) is ignored; (b) **signature** (`_call_targets_same_signature`) — argument arity must match, and when several overloads share that arity the inferred argument types (`_infer_argument_type`: literals, `new X()`, casts, local variables and parameters) must equal the caller's own parameter types; an undecidable type makes the rule **abstain**. Delegation `sync(id)` → `sync(id, false)` is therefore no longer reported. Stripping runs with `keep_string_markers=True` so that `f('a')` is not mistaken for `f()` when counting arity; (c) **guard** — skipped when the method actually **reads** a recursion guard (`_find_recursion_guards` + `_body_is_guarded`): a non-`final` `static` field of type `Boolean`/`Integer`/`Set<>`/`Map<>`, or a tell-tale identifier (`isRunning`, `bypass`, `TriggerBypass.skipAll`, …), genuinely consulted in an `if`/`while` or through `contains(`. The guard is evaluated **per method**: a guard set in another method does not stop recursion inside this one. Each finding carries the offending **signature** (`walk(Id, Integer)`) rather than just the name. |
| 4 | **Trigger after-save recursion detection** (TRIG-REL-001) | `apex_analyzer._detect_trigger_after_save_recursion` | Parses the trigger declaration (`trigger X on Y (after insert, ...)`) then uses regex to check whether `Trigger.new`/`Trigger.newMap` (or an alias assigned via `Object x = Trigger.new`) undergoes a direct DML — the classic infinite-loop signature. Skipped when the trigger body reads a recursion guard (same `_body_is_guarded` logic as rule #3). |
