...
import/export of the extra fields (``csv_io``), report generation
(``generation``) and the per-field "extra info" panel (``field_info``).
This module keeps the window lifecycle, the available/selected object
lists and the per-object "extra info" comment panel (Commentaire Dewey /
Piloté par / Status / Squad Responsable / Squad Consommatrice).
"""
