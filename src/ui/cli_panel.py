...
def build_panel(app: Application, parent: ttk.Frame) -> None:
    ...
    app.cli_frame = ttk.LabelFrame(parent, padding=theme.SPACE_MD)
    app.cli_frame.pack(fill="x", pady=(0, theme.SPACE_MD))

    login_row = ttk.Frame(app.cli_frame)
    login_row.pack(fill="x", pady=(0, theme.SPACE_SM))
    app.alias_label = ttk.Label(login_row, width=14)
    app.alias_label.pack(side="left")
    ttk.Entry(login_row, textvariable=app.alias_var, width=24).pack(
    ...
    app.login_target_combo.pack(side="left", padx=(0, theme.SPACE_MD))
    ...
    app.instance_url_entry.pack(side="left", fill="x", expand=True, padx=(0, theme.SPACE_SM))
    app.login_button = app._track_button(
        ttk.Button(login_row, style=theme.PRIMARY_BUTTON, command=app._login_web)
    )
    ...
    app.org_combo.pack(side="left", fill="x", expand=True, padx=(0, theme.SPACE_SM))
    ...
    app.refresh_button.pack(side="left", padx=(0, theme.SPACE_SM))
    app.generate_manifest_button = app._track_button(
        ttk.Button(org_row, command=app._generate_manifest)
    )
    app.generate_manifest_button.pack(side="left", padx=(0, theme.SPACE_SM))
    app.retrieve_button = app._track_button(
        ttk.Button(org_row, command=app._retrieve_from_selected_org)
    )
    app.retrieve_button.pack(side="left", padx=(0, theme.SPACE_SM))

    app.delete_button = app._track_button(
        ttk.Button(org_row, style=theme.DANGER_BUTTON, command=app._delete)
    )
