...
class Application(tk.Tk):
    ...
    * :mod:`src.ui.settings` for load/save of ``app_settings.json``.
    ...
    def _load_data_model_thresholds(
        self, settings: dict[str, Any]
    ) -> tuple[int, int, int]:
        from src.core.models import DEFAULT_DATA_MODEL_THRESHOLDS

        return parse_thresholds(
            settings, "data_model_thresholds", DEFAULT_DATA_MODEL_THRESHOLDS
        )

    def _load_profiles_thresholds(
        self, settings: dict[str, Any]
    ) -> tuple[int, int, int]:
    ...
    def ai_expand_text(self, original_text: str, callback: Callable[[str], None]) -> None:
        """Use AI to professionalize the provided text."""
        if not original_text.strip():
            return

        provider = self.ai_provider_var.get().strip() or self.AI_PROVIDERS[0]
        if provider == "Claude":
            key = self.claude_api_key_var.get().strip()
        elif provider == "Gemini":
            key = self.gemini_api_key_var.get().strip()
        elif provider == "Gateway":
            key = self.gateway_api_key_var.get().strip()
        else:
            key = ""
        ...
        # Show a "thinking" dialog
        progress_dialog = tk.Toplevel(self)
        progress_dialog.title(self._t("ai_expand_title"))
        progress_dialog.geometry("300x100")
        self._configure_secondary_window(progress_dialog)
        ttk.Label(progress_dialog, text=self._t("discussion_thinking", provider=provider)).pack(pady=20)
        ...
        def worker():
            try:
                ...
                reply = service.chat(messages, system_prompt="Tu es un expert Salesforce chevronné. Tu rédiges des notes techniques concises, professionnelles et percutantes.")
                
                self.after(0, lambda: self._show_ai_expand_result(reply, callback, progress_dialog))
            except Exception as e:
                error_msg = str(e)
                self.after(0, lambda: [
                    progress_dialog.destroy(),
                    messagebox.showerror(self._t("error_title"), f"Erreur IA : {error_msg}")
                ])

        Thread(target=worker, daemon=True).start()

    def _show_ai_expand_result(self, new_text: str, callback: Callable[[str], None], progress_dialog: tk.Toplevel) -> None:
        progress_dialog.destroy()
        ...
        ttk.Label(main_frame, text=self._t("ai_expand_title"), font=("Segoe UI", 12, "bold")).pack(anchor="w", pady=(0, 10))
        ...
        ttk.Button(btn_frame, text=self._t("ai_expand_use"), command=use).pack(side="right")
        ttk.Button(btn_frame, text=self._t("ai_expand_discard"), command=result_window.destroy).pack(side="right", padx=(0, 8))
